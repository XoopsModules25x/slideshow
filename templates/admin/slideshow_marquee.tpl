...
<table id="xo-item-sort" class="outer" cellspacing="1" width="100%">
    ...
    <tr class="odd" id="mod_<{$item.item_id}>">
        ...
        <td class="txtcenter width10 xo-actions">
            <img class="tooltip" onclick="display_dialog(<{$item.item_id}>, true, true, 'slide', 'slide', 300, 700);" src="<{xoAdminIcons display.png}>" alt="<{$smarty.const._PREVIEW}>" title="<{$smarty.const._PREVIEW}>" />
            <a href="marquee.php?op=edit_item&amp;item_id=<{$item.item_id}>"><img class="tooltip" src="<{xoAdminIcons edit.png}>" alt="<{$smarty.const._EDIT}>" title="<{$smarty.const._EDIT}>"/></a>
            <a href="marquee.php?op=delete_item&amp;item_id=<{$item.item_id}>"><img class="tooltip" src="<{xoAdminIcons delete.png}>" alt="<{$smarty.const._DELETE}>" title="<{$smarty.const._DELETE}>"/></a>
        </td>
    </tr>
    <{/foreach}>
    </tbody>
...
<{foreach item=item from=$items}>
	...
	<div class="marg5 pad5 ui-state-highlight ui-corner-all">
	   <div class="pad5"><span class="bold"><{$smarty.const._AM_SLIDESHOW_ITEM_TITLE}> : <{$item.item_title}></span></div>
		<div class="pad5"><span class="bold"><{$smarty.const._AM_SLIDESHOW_ITEM_LINK}> : </span><{$item.item_link}></div>
		<div class="clear"></div>
   </div>
	</div>
<{/foreach}>
